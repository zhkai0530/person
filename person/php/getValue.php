	 ...
	 */
 	class register {
 		...
 		public function checkEmail(){
 			if(!trim($this->email)==""){
 				...
 				if(!preg_match($preg,$this->email)){
 					...
 					$sql = "select * from personinfo where email='".$this->email."'";
 		...
 		public function checkPhone(){
 			if(!trim($this->phone)==""){
 				...
 				if(!preg_match($preg,$this->phone)){
 					...
 					$conn = new mysqli(DB_HOST,DB_USER,DB_PWD,DB_NAME) or die("数据库连接异常");
 					mysqli_query($conn,"set names utf8");
 					$sql = "select * from personinfo where phone='".$this->phone."'";
 					$result = mysqli_query($conn,$sql);
 					if(mysqli_num_rows($result)>0){
 						echo "<script>alert(\"手机号已被注册，请更换其他手机！\");history.go(-1);</script>";
 						exit();
 					}
 				}
 			}
 		...
 		public function inject_info_in_sql(){
 			...
 			mysqli_query($conn,"set names utf8");
 			$sql = "insert into personinfo(nickName,name,phone,email,password) values('".$this->nickName."','".$this->personName."','".$this->phone."','".$this->email."','".$this->pwd."')";
